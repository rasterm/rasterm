...
int roundedSixteenth(const int value) noexcept
{
    return value >= 0 ? (value + 8) / 16 : (value - 8) / 16;
}

void mapAdaptiveFloydSteinberg(const FrameView& source, const PixelLayout layout,
                               const AdaptivePaletteMapper& mapper,
                               std::vector<std::uint8_t>& paletteIndices,
                               std::vector<int>& currentError,
                               std::vector<int>& nextError)
{
    const std::size_t errorValues = static_cast<std::size_t>(source.width) * 3;
    currentError.assign(errorValues, 0);
    nextError.assign(errorValues, 0);
    const int sourceStride = pixelStride(layout);
    for (int y = 0; y < source.height; ++y) {
        const bool reverse = (y & 1) != 0;
        const int direction = reverse ? -1 : 1;
        const int end = reverse ? -1 : source.width;
        const std::uint8_t* row = source.data +
            static_cast<std::ptrdiff_t>(y) * source.stride;
        std::uint8_t* mapped = paletteIndices.data() +
            static_cast<std::size_t>(y) * source.width;
        for (int x = reverse ? source.width - 1 : 0; x != end; x += direction) {
            const PixelChannels channels = readPixel(row + x * sourceStride, layout);
            const std::array original{ channels.red, channels.green, channels.blue };
            std::array<int, 3> adjusted{};
            const std::size_t errorOffset = static_cast<std::size_t>(x) * 3;
            for (int channel = 0; channel < 3; ++channel) {
                adjusted[channel] = clampChannel(
                    original[channel] + roundedSixteenth(currentError[errorOffset + channel]));
            }

            const int color = mapper.getColorNumberReadOnly(
                static_cast<std::uint8_t>(adjusted[0]),
                static_cast<std::uint8_t>(adjusted[1]),
                static_cast<std::uint8_t>(adjusted[2]));
            mapped[x] = static_cast<std::uint8_t>(color);
            const std::uint32_t rgb = mapper.palette[color];
            const std::array paletteColor{
                static_cast<int>((rgb >> 16) & 0xFF),
                static_cast<int>((rgb >> 8) & 0xFF),
                static_cast<int>(rgb & 0xFF),
            };

            const int adjacent = x + direction;
            const int lowerOpposite = x - direction;
            for (int channel = 0; channel < 3; ++channel) {
                const int error = adjusted[channel] - paletteColor[channel];
                if (adjacent >= 0 && adjacent < source.width) {
                    currentError[static_cast<std::size_t>(adjacent) * 3 + channel] += error * 7;
                    nextError[static_cast<std::size_t>(adjacent) * 3 + channel] += error;
                }
                nextError[errorOffset + channel] += error * 5;
                if (lowerOpposite >= 0 && lowerOpposite < source.width) {
                    nextError[static_cast<std::size_t>(lowerOpposite) * 3 + channel] += error * 3;
                }
            }
        }
...
bool sixelAvx2Enabled(const int width) noexcept
{
    const int mode = simdMode.load(std::memory_order_relaxed);
    constexpr int automaticCrossoverWidth = (std::numeric_limits<int>::max)();
    return (mode == 1 || (mode == -1 && width >= automaticCrossoverWidth)) &&
        sixelAvx2Supported();
}

void setSixelSimdModeForTesting(const int mode) noexcept
{
    simdMode.store(std::clamp(mode, -1, 2), std::memory_order_relaxed);
}

SixelEncoder::SixelEncoder(const SixelOptions& requestedOptions) : options(requestedOptions)
{
    ...
    analyzer = std::make_unique<ColorAnalyzer>();
    adaptivePaletteMapper = std::make_unique<AdaptivePaletteMapper>(activeColorSlots);
    fixedPaletteMapper = std::make_unique<FixedPaletteMapper>(activeColorSlots);
...
bool sixelAvx512Enabled(const int width) noexcept
{
    const int mode = simdMode.load(std::memory_order_relaxed);
    constexpr int automaticCrossoverWidth = (std::numeric_limits<int>::max)();
    return (mode == 2 || (mode == -1 && width >= automaticCrossoverWidth)) &&
        sixelAvx512Supported();
}

void SixelEncoder::prepareFrame(const FrameView& frame)
{
    const auto analysisStart = std::chrono::steady_clock::now();
    pendingAnalysisDuration = {};
    beginLogicalFrame();
    adaptiveFramePrepared = false;
    if (!options.useAdaptivePalette || !frame.isValid()) return;

    analyzer->analyzeFrame(frame, pixelLayout(frame.format));
    float sceneDifference = 1.0f;
    if (adaptivePaletteMapper->hasHistogram) {
        ...
        for (std::size_t index = 0; index < analyzer->luminanceHistogram.size(); ++index) {
            sceneDifference += std::abs(analyzer->luminanceHistogram[index] -
                                        adaptivePaletteMapper->previousHistogram[index]);
        }
    ...
    if (!hasAdaptivePalette || paletteAge >= options.adaptivePaletteLockFrames ||
        ...
        adaptivePaletteMapper->buildOptimalPalette(*analyzer);
        paletteAge = 0;
    ...
    pendingAnalysisDuration = std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::steady_clock::now() - analysisStart);
}

void SixelEncoder::prepareFrame(const IndexedFrameView&)
{
    pendingAnalysisDuration = {};
    beginLogicalFrame();
    adaptiveFramePrepared = false;
...
void SixelEncoder::prepareRegionalFrame(const FrameView& frame)
{
...
void SixelEncoder::prepareRegion(const FrameView& frame, const DamageRegion& region)
{
    pendingAnalysisDuration = {};
    if (!options.useAdaptivePalette || !options.independentRegionQuantization ||
    ...
    analyzer->analyzeFrame(view, pixelLayout(view.format));
    adaptivePaletteMapper->buildOptimalPalette(*analyzer);
    paletteAge = 0;
    hasAdaptivePalette = true;
    adaptiveFramePrepared = true;
...
void SixelEncoder::beginLogicalFrame() noexcept
{
...
bool SixelEncoder::shouldEmitPalette(const std::uint64_t signature) noexcept
{
...
void SixelEncoder::reset() noexcept
{
    paletteAge = 0;
    hasAdaptivePalette = false;
    adaptiveFramePrepared = false;
    invalidatePaletteRegisters();
    adaptivePaletteMapper->hasHistogram = false;
    adaptivePaletteMapper->rgbToColorNum.clear();
}

void SixelEncoder::invalidatePaletteRegisters() noexcept
{
...
std::string_view SixelEncoder::encodeFrame(const FrameView& frame)
{
    return frame.isValid() ? encodeView(frame, pixelLayout(frame.format)) : std::string_view{};
}

std::string_view SixelEncoder::encodeFrame(const IndexedFrameView& frame)
{
    return frame.isValid() ? encodeView(frame) : std::string_view{};
}

std::string_view SixelEncoder::encodeRegion(const FrameView& frame, const DamageRegion& region)
{
...
std::string_view SixelEncoder::encodeRegion(const IndexedFrameView& frame,
...
std::string_view SixelEncoder::encodeView(const FrameView& frame, const PixelLayout layout)
{
    stageTimings = {};
    stageTimings.analysis = pendingAnalysisDuration;
    pendingAnalysisDuration = {};
    limitExceeded = false;
    const auto mappingStart = std::chrono::steady_clock::now();
    FrameView source = frame;
    PixelLayout sourceLayout = layout;
    const bool useGenericDither = options.dither != DitherMode::None &&
        (!options.useAdaptivePalette || options.dither != DitherMode::FloydSteinberg);
    if (useGenericDither) {
        ...
        if (options.dither == DitherMode::FloydSteinberg) {
            applyFloydSteinberg(workingBuffer, frame.width, frame.height,
                                options.ditherLevelsPerChannel);
        }
        else {
    ...
    if (options.useAdaptivePalette) {
        if (!adaptiveFramePrepared) {
            const auto analysisStart = std::chrono::steady_clock::now();
            analyzer->analyzeFrame(source, sourceLayout);
            float sceneDifference = 1.0f;
            if (adaptivePaletteMapper->hasHistogram) {
                sceneDifference = 0.0f;
                for (std::size_t index = 0; index < analyzer->luminanceHistogram.size(); ++index) {
            ...
            stageTimings.analysis += std::chrono::duration_cast<std::chrono::nanoseconds>(
                std::chrono::steady_clock::now() - analysisStart);
        }
        colorCount = adaptivePaletteMapper->nextColorNum;
        if (options.dither == DitherMode::FloydSteinberg) {
            mapAdaptiveFloydSteinberg(source, sourceLayout, *adaptivePaletteMapper,
                                      paletteIndices, currentDitherError, nextDitherError);
        }
        else {
            struct AdaptiveMapContext {
                const FrameView* source;
                PixelLayout layout;
                AdaptivePaletteMapper* mapper;
                std::uint8_t* indices;
            } context{ &source, sourceLayout, adaptivePaletteMapper.get(), paletteIndices.data() };
        ...
        fixedPaletteMapper->reset();
        const bool simdEligible = pixelStride(sourceLayout) >= 3 && source.width >= 64;
        const int dispatchMode = simdEligible && sixelAvx512Enabled(source.width) ? 2
            : simdEligible && sixelAvx2Enabled(source.width) ? 1 : 0;
        struct FixedMapContext {
            const FrameView* source;
            PixelLayout layout;
            const FixedPaletteMapper* mapper;
            std::uint8_t* indices;
            int simd;
        } context{
            &source, sourceLayout, fixedPaletteMapper.get(), paletteIndices.data(), dispatchMode
        };
        const auto mapRow = +[](void* raw, const int y) noexcept {
        ...
        for (int pixel = 0; pixel < totalPixels; ++pixel) {
            ...
            if (!fixedPaletteMapper->colorUsed[color]) {
                fixedPaletteMapper->colorUsed[color] = true;
                ++fixedPaletteMapper->usedColorCount;
            }
        }
        colorCount = fixedPaletteMapper->nextColorNum;
    }

    stageTimings.mapping = std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::steady_clock::now() - mappingStart);

    const bool previousPaletteRegistersValid = paletteRegistersValid;
    const std::uint64_t previousPaletteSignature = paletteSignature;
    const int previousFramesSinceRefresh = framesSincePaletteRefresh;
    ...
    if (options.useAdaptivePalette) {
        const bool definePalette = shouldEmitPalette(paletteHash(
            adaptivePaletteMapper->palette.data(),
            static_cast<std::size_t>(adaptivePaletteMapper->nextColorNum)));
        if (definePalette) emitPalette(output, *adaptivePaletteMapper);
    }
    else {
        const bool definePalette = shouldEmitPalette(paletteHash(
            fixedPaletteMapper->colorNumToRgb.data(),
            static_cast<std::size_t>(fixedPaletteMapper->nextColorNum)));
        if (definePalette) {
            if (options.persistPaletteRegisters) emitFullPalette(output, *fixedPaletteMapper);
            else emitPalette(output, *fixedPaletteMapper);
        }
    }
    emitIndexedFrame(output, paletteIndices, source.width, source.height, colorCount);
    output.append("\x1b\\");
    stageTimings.writing = std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::steady_clock::now() - writingStart);
    const auto budgetingStart = std::chrono::steady_clock::now();
    limitExceeded = output.exceeded();
    if (limitExceeded) {
        outputBuffer.clear();
        paletteRegistersValid = previousPaletteRegistersValid;
        paletteSignature = previousPaletteSignature;
        framesSincePaletteRefresh = previousFramesSinceRefresh;
    }
    stageTimings.budgeting = std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::steady_clock::now() - budgetingStart);
    lastColorCountValue = options.useAdaptivePalette
        ? colorCount - 1
        : fixedPaletteMapper->usedColorCount;
    return limitExceeded ? std::string_view{} : std::string_view(outputBuffer);
}

std::string_view SixelEncoder::encodeView(const IndexedFrameView& frame)
{
    stageTimings = {};
    stageTimings.analysis = pendingAnalysisDuration;
    pendingAnalysisDuration = {};
...
std::size_t SixelEncoder::scratchCapacity() const noexcept
{
    return paletteIndices.capacity() + workingBuffer.capacity() +
        (currentDitherError.capacity() + nextDitherError.capacity()) * sizeof(int);
}

SixelEncoder::~SixelEncoder() = default;
